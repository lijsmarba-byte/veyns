...
const OVERLAY_SOFT_BACKGROUND =
  "radial-gradient(130% 118% at 50% 50%, rgba(254,254,253,0.97) 48%, rgba(254,254,253,0.93) 68%, rgba(254,254,253,0.7) 84%, rgba(254,254,253,0.22) 100%)";
...
export function ProfileSignatureContourInline({ user }: { user: MockUserProfile }) {
  ...
  const activeCardPlacement = useMemo(() => {
    ...
    const rawLeft = activeCluster.cx - overlayCardWidth * 0.5;
    const rawTop = activeCluster.cy - OVERLAY_TITLE_ANCHOR_Y;

    const clampedLeft = clamp(rawLeft, OVERLAY_CARD_PADDING, WIDTH - overlayCardWidth - OVERLAY_CARD_PADDING);
    const clampedTop = clamp(rawTop, OVERLAY_CARD_PADDING, HEIGHT - overlayCardHeight - OVERLAY_CARD_PADDING);

    return {
      leftPercent: (clampedLeft / WIDTH) * 100,
      topPercent: (clampedTop / HEIGHT) * 100,
    } as const;
  }, [activeCluster, overlayCardHeight, overlayCardWidth]);
            ...
            return (
              <text
                ...
                style={{
                  ...
                  fontWeight: 500,
                  fill: isActive || isHovered ? "var(--ink)" : "var(--meta)",
            ...
            >
              ...
              <div className="relative mx-auto w-full max-w-[560px]">
                <h3 className="flex w-full items-baseline justify-start leading-none text-ink">
                  ...
                  <span className="-ml-[1px] font-ui text-[22px] font-normal leading-none tracking-[-0.06em]">–</span>
                  <span className="ml-[2px] font-instrument text-[22px] italic leading-none tracking-[0.01em]">
                    {activeCluster.name}
                  </span>
                </h3>

                <p className="mt-5 text-justify font-ui text-[14px] font-normal leading-[1.8] text-meta">
                  {thesisText}
                </p>
                ...
                <div className="mx-auto mt-6 flex max-w-[470px] flex-wrap justify-center gap-x-[14px] gap-y-[10px]">
                  {focusAttributes.map((word) => (
                    <span
                      ...
                      className="inline-flex h-[29px] items-center justify-center whitespace-nowrap rounded-[999px] border border-[#6F7381] bg-[#6F7381] px-[11px] text-center font-ui text-[14px] font-normal leading-[18px] tracking-[-0.03em] text-paper shadow-[0_1px_2px_rgba(0,0,0,0.12)]"
                    >
                      {word.label}
                    </span>
                  ))}
