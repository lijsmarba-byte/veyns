...
export function ProfileSignatureContent({ user }: { user: MockUserProfile }) {
  ...
  const [dimmedLabelIds, setDimmedLabelIds] = useState<Set<string>>(new Set());
  ...
  const thesisText = limitToSentences(activeCluster ? activeCluster.thesis.trim() : "", 2);
  const tagsInline = focusAttributes.map((word) => word.label).join(" | ");
  const topTasteText = user.tasteDescription.tasteThesis.trim();
  const typingTargetText = topTasteText;
  ...
  useEffect(() => {
    const compute = () => {
      if (!activeCluster || !mapSvgRef.current || !detailCardRef.current) {
        setDimmedLabelIds(new Set());
        return;
      }
      ...
      for (const cluster of data.clusters) {
        if (cluster.id === activeCluster.id) continue;

        const labelCenterX = svgRect.left + cluster.cx * scaleX;
        const labelCenterY = svgRect.top + cluster.cy * scaleY;
        const labelW = cluster.labelW * scaleX;
        const labelH = cluster.labelH * scaleY;
        const left = labelCenterX - labelW / 2;
        const right = labelCenterX + labelW / 2;
        const top = labelCenterY - labelH / 2;
        const bottom = labelCenterY + labelH / 2;
        ...
        if (overlaps) next.add(cluster.id);
      }

      setDimmedLabelIds(next);
    };
      ...
      {!isUnlocked ? (
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.9, ease: EASE }}>
          ...
          {isGraphRevealReady ? (
            <motion.section
              ref={mapSectionRef}
              className="relative mx-auto max-w-[680px] bg-[#FCFCFA] p-2"
              initial={{
                opacity: 0,
                y: 56,
                scale: 0.95,
                    ...
                    return (
                      <text
                        ...
                        style={{
                          ...
                          opacity: activeClusterId && dimmedLabelIds.has(cluster.id) ? 0.3 : 1,
                          letterSpacing: "0.015em",
                          cursor: "pointer",
                          transition: "fill 520ms cubic-bezier(0.22,1,0.36,1), opacity 200ms ease",
                        }}
                      >
                        {cluster.name}
                      </text>
            ...
            <AnimatePresence>
              {activeCluster ? (
                ...
                >
                  <div
                    ref={detailCardRef}
                    className="pointer-events-auto relative w-full max-w-[600px] max-h-[78%] overflow-y-auto rounded-[2px] border border-line bg-paper p-8"
                  >
                    <button
                      type="button"
                    ...
                    <h2 className="inline-flex w-full max-w-[620px] items-center justify-start text-left leading-none text-ink">
                      <span className="font-ui text-[14px] font-normal">The–</span>
                      <span className="ml-1 font-instrument text-[28px] italic">{activeCluster.name}</span>
                    </h2>

                    <p className="mt-4 max-w-[540px] text-justify font-ui text-[14px] font-normal leading-[1.7] text-meta">
                      {thesisText}
                    </p>

                    <p className="mt-4 max-w-[540px] font-ui text-[12px] font-normal tracking-[0.02em] text-meta">
                      {tagsInline}
                    </p>
                  </div>
                </motion.div>
