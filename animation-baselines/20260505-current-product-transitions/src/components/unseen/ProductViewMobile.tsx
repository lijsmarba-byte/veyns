    ...
    >
      ...
      {isSaved ? (
        <button
          ...
          aria-label={isArchiveMode ? "Saved" : "Remove from saved"}
        >
          saved
        </button>
      ) : !isSaveFlowOpen ? (
...
export function ProductViewMobile({
  ...
  return (
    <main
      ...
      onClick={(event) => {
        if (Date.now() - mountedAtRef.current < MOBILE_BACKDROP_CLOSE_ARM_DELAY_MS) return;
    ...
    >
      ...
      >
        ...
        <section
          data-pv-mobile-hit="true"
          className="row-start-2 min-h-0 min-w-0 overflow-hidden"
          style={{ transform: "translateY(-16px)" }}
        >
          <div
            className="relative h-full w-full min-w-0 touch-pan-y select-none overflow-hidden"
          ...
          >
            ...
            {hasMobileCues && activeImageIndex === 0 ? (
              <span
                ...
                className={`pointer-events-none absolute left-0 top-0 z-30 inline-flex h-[29px] items-center whitespace-nowrap rounded-[999px] border px-[11px] shadow-[0_0_0_0.5px_rgba(17,17,17,0.22)] transition-opacity duration-180 ease-out ${
        ...
        <section data-pv-mobile-hit="true" className="row-start-4 flex min-h-0 min-w-0 w-full flex-col overflow-hidden pt-[2px]">
          <h1 className={`relative inline-flex w-full min-w-0 items-baseline justify-start overflow-visible whitespace-nowrap text-left text-[28px] leading-none ${titleToneClass}`}>
            <span className={`inline-flex items-center font-ui text-[26px] font-normal leading-none tracking-[-0.06em] ${titleToneClass}`}>
              <span aria-hidden="true" className="inline-flex items-center text-[24px] leading-none">|</span>
              <span className="px-[2px]">{pipeLabel}</span>
              <span aria-hidden="true" className="inline-flex items-center text-[24px] leading-none">|</span>
            </span>
            <span className={`-ml-[3px] pl-[1px] pr-[2px] font-ui text-[26px] font-normal leading-none tracking-[-0.06em] ${titleToneClass}`}>–</span>
